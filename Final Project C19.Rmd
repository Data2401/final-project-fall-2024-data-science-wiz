...
covid19_data %>% filter(Province_State == "All States")
```
...
new_covid19_data <- full_join(c19_dd, covid19_data, by = join_by(date)) 
...
Final_covid19_data %>% filter(month == "2") %>% arrange(-positive)
```

```{r}
#this section im trying to filter out the US Data 
new_covid19_data %>% filter(month == "2") %>% arrange(-positive) %>% select(month, Country_Region, positive) 

US_data <- new_covid19_data %>% filter(Country_Region == "United States", Province_State == "All States")

US_data %>% arrange(-positive) 

US_data[!duplicated(US_data$day), ]
#shows how many duplicated rows of each
